...
- 対象者のプロフィールへ別タブで移動する
- 対象者の最新オリジナル投稿に、現在ログインしているユーザーアカウントとして自然なリプを作成する
...
- 複数人分を一度に処理しない。ただし、ユーザーが「3件」「5件」など件数を指定した場合は、その件数分だけ順番に処理してよい
...
- リプ周りでは、返信文を入力しただけで止めず、Xの「下書き」に保存する
- 直前に処理した相手より上に戻らない
...
- 実際に送信しない
- 返信ボタンを押さない
- 複数人分をまとめて作らない。ただし、ユーザーが件数を指定した場合は、その件数分だけ順番に処理してよい
- 順番を飛ばさない
- 元ポストのタブを閉じない
- リプ周りを同じタブ内で進めない
